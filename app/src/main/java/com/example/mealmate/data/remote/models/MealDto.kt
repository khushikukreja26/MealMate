...
data class MealDto(
    ...
    /** Old helper (kept, used elsewhere) */
    fun ingredients(): List<String> {
        return ingredientPairs().map { (i, m) ->
            if (m.isNotBlank()) "$i — $m" else i
        }
    }

    /** 🔑 New helper for table: pairs of (Ingredient, Measure), blanks filtered & trimmed */
    fun ingredientPairs(): List<Pair<String, String>> {
        ...
        return pairs
            .mapNotNull { (i, m) ->
                val ing = (i ?: "").trim()
                val meas = (m ?: "").trim()
                if (ing.isNotEmpty()) ing to meas else null
            }
    }
}